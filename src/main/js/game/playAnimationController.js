...
define([
  ...
  // SPORTSPEC-133 - LNB: The game is displaying X3 win when its going giving the user a X2 win.
  ...
  function handleInitialAnimComplete() {
    ...
    spinButton.enable(initialAnimComplete);
    autoSpinButton.enable(initialAnimComplete && autoRevealEnabled);
  ...
  function onStartUserInteraction(data) {
    ...
    if (autoRevealEnabled) {
      autoSpinButton.enable(initialAnimComplete);
  ...
  // SPORTSPEC-134: LNB: When Hardwar acceleration is off the Picker bonus is not adding to the game
  ...
  function startBase(data) {
    ...
    gr.lib._0_collectWin.setText(
      SKBeInstant.formatCurrency(data.money).formattedAmount.trim()
    );
    isBonus = false;
    gr.getTimer().setTimeout(function () {
      if (isLastSpin()) {
